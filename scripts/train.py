...
def make_cfg(args):
    ...
    with open(args.config, encoding="utf-8") as f:
        ...
        if "fix_truncation" in config.keys():
            cfg.fix_truncation = config["fix_truncation"]
        ...
        if "use_rootnet" in config.keys():
            cfg.use_rootnet = config["use_rootnet"]
            cfg.resample = config["resample"] if "resample" in config else False
            cfg.use_origin_bbox = config["use_origin_bbox"]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--exp_name', '-e', type=str, help="The checkpoints and logs will be save in ./experiments/$EXP_NAME")
    parser.add_argument('--config', '-c', type=str, default='configs/cfg.yaml', help="hyperparameters path")
    parser.add_argument('--gpu', '-g', type=str, help="gpu number, 456 -> gpu 4,5,6")
    parser.add_argument('--n_epochs', '-t',type=int, help="total epochs")
    parser.add_argument('--batch_size', '-b',type=int, help="batch size")
    parser.add_argument('--lr','-l', type=float, help="Learning rate")
    parser.add_argument('--robot','-r', type=str, help="robot type name")
    parser.add_argument('--dataset_path','-p', type=str, help="urdf dataset path")

    args = parser.parse_args()
    
    # if input exp_name, total_epoch, batch_size, learning_rate, robot, dataset path, gpu by hand
    # the settings in config yaml file will be covered
    cfg = make_cfg(args)
    print(f"Using {cfg.train_ds_names} for training .")
    print("image size settings:")
    print(f"Using image size of {cfg.image_size}")
    print(f"Using rootnet image size: {cfg.rootnet_image_size}")
    print(f"Using other image size: {cfg.other_image_size}")
    print(f"Using batchsize {cfg.batch_size} and lr {cfg.lr} and lr scheduling {cfg.use_schedule} .")
    if cfg.use_schedule:
        print(f"Using scheduling type {cfg.schedule_type} and starting from epoch {cfg.start_decay}")
    if cfg.clip_gradient is not None:
        print(f"using gradient clip, clipping value: {cfg.clip_gradient}")
    else:
        print("not using gradient clip")
    print(f"fixing truncation: {cfg.fix_truncation}")
    print(f"padding for all: {cfg.padding}")
    
    if cfg.use_sim2real:
        print("using sim2real training on real datasets")
        train_sim2real(cfg)
        exit()
    
    if cfg.use_sim2real_real:
        print("using sim2real training on real-world datasets")
        # train_sim2real_real(cfg)
        exit()

    if cfg.use_rootnet:
        print(f"use backbone: {cfg.backbone_name}")
        print(f"use original bbox: {cfg.use_origin_bbox}")
        print(f"use extended bbox: {cfg.use_extended_bbox}")
        if cfg.use_extended_bbox:
            print(f"extend ratio: {cfg.extend_ratio}")
        print(f"use offset layer: {cfg.use_offset}")
        if cfg.use_rootnet_with_reg_int_shared_backbone:
            print(f"jointly training rootnet with regression+integral, full pipeline, 2 backbone")
            train_full(cfg)
        if cfg.use_rootnet_with_reg_with_int_separate_backbone:
            print(f"jointly training rootnet with regression and integral, full pipeline, 3 backbone")
            train_full(cfg)
        if cfg.use_rootnet_reg_int_all_shared_backbone:
            print(f"jointly training rootnet, regression and integral in 1 backbone")
            train_full(cfg)

        else:
            print("training rootnet only")
            train_depthnet(cfg)
        exit()
