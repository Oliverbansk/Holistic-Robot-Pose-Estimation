...
from lib.config import (BAXTER_DESCRIPTION_PATH, KUKA_DESCRIPTION_PATH,
                        LOCAL_DATA_DIR, PANDA_DESCRIPTION_PATH)
...
from lib.dataset.dream_real import DreamRealDataset
from lib.dataset.multiepoch_dataloader import MultiEpochDataLoader
from lib.dataset.samplers import PartialSampler
from lib.models.ctrnet.mask_inference import seg_mask_inference, seg_keypoint_inference
...
from lib.utils.utils import cast, create_logger, get_scheduler, set_random_seed
from lib.utils.vis import vis_3dkp_single_view
from torch.utils.data import DataLoader
from torch.utils.tensorboard import SummaryWriter
from torchnet.meter import AverageValueMeter
from tqdm import tqdm
from unit_test.sam.seg import get_sammask


def train_sim2real_real(args):
    ...
    test_ds_name_real = args.train_ds_names
    ...
    ds_train = DreamRealDataset(train_ds_names, rootnet_resize_hw=rootnet_hw, other_resize_hw=other_hw)
    ...
    ds_test = DreamRealDataset(test_ds_name_real, rootnet_resize_hw=rootnet_hw, other_resize_hw=other_hw) 
    ds_iter_test = DataLoader(
        ds_test, batch_size=args.batch_size, num_workers=args.n_dataloader_workers,
    )
    
    print("len(ds_iter_train): ",len(ds_iter_train))
    print(f"len(ds_iter_test): ", len(ds_iter_test))
    ...
    print("do self-supervised training, based on the full network")
    model = get_rootNetwithRegInt_model(init_param_dict, args)
                                     
    seg_net = seg_mask_inference(INTRINSICS_DICT["azure"], "move_arm")
    seg_kp_net = seg_keypoint_inference()
    
    # only tune final layer of jointnet rotationnet
    # for param in model.parameters():
    #     param.requires_grad = False
    # for param in model.fc_pose_1.parameters():
    #     param.requires_grad = True
    # for param in model.fc_pose_2.parameters():
    #     param.requires_grad = True
    # for param in model.decpose.parameters():
    #     param.requires_grad = True
    # for param in model.fc_rot_1.parameters():
    #     param.requires_grad = True
    # for param in model.fc_rot_2.parameters():
    #     param.requires_grad = True
    # for param in model.decrot.parameters():
    #     param.requires_grad = True
    ...
    for epoch in range(start_epoch, end_epoch + 1):
        ...
        def farward_loss(args,input_batch, device, model, train=True, save=False, batchid=None, epoch_log=None, vis_step=None, view_ids=None, errors=None):
            ...
            gt_pose = torch.ones((batch_size,8)).to(torch.float32).cuda()
            gt_rot = torch.stack(gt_rot, 0).to(torch.float32).cuda()
            gt_trans = torch.stack(gt_trans, 0).to(torch.float32).cuda()
            ...
            if train:
                ...
                keypoints, seg = seg_kp_net(images_original_255)
                image_for_sam = [cv2.resize(image_ori.permute(1,2,0).cpu().numpy().astype(np.uint8),(320,240)) for image_ori in images_original_255]
                keypoint_for_sam = keypoints.detach().cpu().numpy().astype(int)
                rendered_masks = []
                seg_masks = []
                for i in range(batch_size):
                    ...
                    seg_masks.append(torch.tensor(get_sammask(image_for_sam[i], keypoint_for_sam[i]), requires_grad=False))
                if args.use_view:
                    if batchid < 2:
                ...
                seg_masks = torch.cat(seg_masks, 0).to(torch.float).cuda()
                ...
                si = seg_masks[0].reshape(240,320).detach().cpu().numpy().astype(np.uint8) * 255
                cv2.imwrite(f'unit_test/view3/segmentation.jpg', np.uint8(si))
        ...
        def validate(ds="test", epoch_log=0, get_lowest=False, view_ids=None, errors=None):
            get_lowest = False
            to_save = False
            step = 40
            loader = ds_iter_test
        ...
        for batchid, sample in enumerate(iterator):
            ...
            if (batchid+1) % 3 == 0:    # Every 100 mini-batches/iterations
