...
import utils.rpmg_tools as rpmg_tools
...
class RPMG(torch.autograd.Function):
    ...
    def forward(ctx, in_nd, tau, lam, rgt, iter, weight=1):
        ...
        if proj_kind == 6:
            r0 = rpmg_tools.compute_rotation_matrix_from_ortho6d(in_nd)
        elif proj_kind == 9:
            r0 = rpmg_tools.symmetric_orthogonalization(in_nd)
        elif proj_kind == 4:
            r0 = rpmg_tools.compute_rotation_matrix_from_quaternion(in_nd)
        elif proj_kind == 10:
            r0 = rpmg_tools.compute_rotation_matrix_from_10d(in_nd)
        else:
    ...
    def backward(ctx, grad_in):
        ...
        if tau == -1:
            ...
            if iter % 100 == 0:
                ...
                R0_Rgt = rpmg_tools.compute_geodesic_distance_from_two_matrices(r0, rgt)
        ...
        if proj_kind == 6:
            ...
            SVD_proj = rpmg_tools.compute_SVD_nearest_Mnlsew(in_nd.reshape(-1,3,3), r_new)
            gradient_nd = in_nd - SVD_proj + lam * (SVD_proj - r_new.reshape(-1,9))
            R_proj_g = rpmg_tools.symmetric_orthogonalization(SVD_proj)
            if iter % 100 == 0:
            ...
            q_1 = rpmg_tools.compute_quaternions_from_rotation_matrices(r_new)
            q_2 = -q_1
            normalized_nd = rpmg_tools.normalize_vector(in_nd)
            q_new = torch.where(
            ...
            qg = rpmg_tools.compute_quaternions_from_rotation_matrices(r_new)
            new_x = rpmg_tools.compute_nearest_10d(in_nd, qg)
            reg_A = torch.eye(4, device=qg.device)[None].repeat(qg.shape[0],1,1) - torch.bmm(qg.unsqueeze(-1), qg.unsqueeze(-2))
            reg_x = rpmg_tools.convert_A_to_Avec(reg_A)
