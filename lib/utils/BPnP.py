import torch
a = torch.inverse(torch.tensor([[1,0,0],[0,1,0],[0,0,1]], dtype=torch.float, device="cuda"))
del a
import cv2 as cv
import numpy as np
import kornia as kn
...
class BPnP(torch.autograd.Function):
    """
    Back-propagatable PnP --- Adapted from https://github.com/ucsdarclab/CtRNet-robot-pose-estimation/blob/main/models/BPnP.py
    INPUTS:
...
class BPnP_m3d(torch.autograd.Function):
    """
    Adapted from https://github.com/ucsdarclab/CtRNet-robot-pose-estimation/blob/main/models/BPnP.py
    ...
    def backward(ctx, grad_output):
        ...
        for i in range(bs):
            ...
            # start_image_backward = time.time()
            ...
            torch.cuda.synchronize()
            b = torch.inverse(torch.tensor([[1,0,0],[0,1,0],[0,0,1]], dtype=torch.float, device="cuda")) # to prevent "RuntimeError: lazy wrapper should be called at most once"
            b = b + torch.inverse(torch.tensor([[1,0,0],[0,1,0],[0,0,1]], dtype=torch.float, device="cuda"))
            inv_J_fy = torch.inverse(J_fy)
            # U, S, V = torch.svd(J_fy)
            # inv_J_fy = torch.mm(V, torch.mm(torch.diag(1.0 / S), U.t()))
            # P, L, U = torch.lu(J_fy)
            # inv_J_fy = torch.mm(torch.inverse(U), torch.mm(torch.inverse(L), P.t()))
            torch.cuda.synchronize()
            ...
            # torch.cuda.synchronize()
            # end_image_backward = time.time()
        
        # print("fjb time:", (end_fjb - start_fjb) * m * bs)
        # print("image backward time:", (end_image_backward - start_image_backward) * bs)
